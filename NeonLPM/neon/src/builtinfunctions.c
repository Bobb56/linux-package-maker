...
/* Copie récursive d'un dossier ou fichier */
int copy_path(const char *src, const char *dst)
{
    ...
    if (S_ISDIR(st.st_mode)) // dossier
    {
        ...
        while ((entry = readdir(dir)) != NULL)
        {
            ...
            int err = copy_path(src_path, dst_path);
            if (err != 0)
            {
                closedir(dir);
                return err;
    ...
    return EINVAL; // type de fichier non supporté
}




NeObj _copyPath_(NeList* args) {
    copy_path(neo_to_string(ARG(0)), neo_to_string(ARG(1)));
    return neo_none_create();
}
...
int delete_path(char* path) {
    ...
    /* Si c'est un fichier ou un lien symbolique */
...
NeObj _deletePath_(NeList* args) {
    return neo_bool_create(delete_path(neo_to_string(ARG(0))) == 0);
}

NeObj _getHomePath_(NeList* args) {
    struct passwd *pw = getpwuid(getuid());
    if (pw)
        return neo_str_create(strdup(pw->pw_dir));
...
NeObj _makeDirectory_(NeList* args) {
    return neo_bool_create(create_directory_recursive(neo_to_string(ARG(0))) == 0);
}
...
NeObj _makeExecutable_(NeList* args) {
    return neo_bool_create(make_executable(neo_to_string(ARG(0))) == 0);
}

NeObj _getCurrentPath_(NeList* args) {
    char path[PATH_MAX];
    ssize_t len = readlink("/proc/self/exe", path, sizeof(path) - 1);
    if (len == -1) return neo_none_create();

    path[len] = '\0';

    // dirname peut modifier la chaîne → on travaille sur une copie
