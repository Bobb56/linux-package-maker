...
class LPMGui:
    def __init__(self, root):
        self.root = root
        self.root.title("Linux Package Maker - Graphical Assistant")
    ...
    def update_installer_name(self, event=None):
        ...
        directory = LPM.getFileDirectory(self.entries["AppDirectory"].get())
        if name:
        ...
        for key, val in data.items():
            ...
            if key == "DesktopIcon":
                ...
                if key == "AppDirectory" or key == "Icon":
                    val = LPM.solve_relative_path(LPM.getFileDirectory(file), val)
                elif key == "Launcher":
                    val = LPM.solve_relative_path(LPM.getFileDirectory(file), data["AppDirectory"]) + '/' + val
